...
export class ContentScanner {
     ...
     */
    async scanBySlug(slug, mode = 'incremental') {
        const result = {
            imagesProcessed: 0,
            thumbnailsGenerated: 0,
            directoriesCreated: 0,
        ...
        try {
            ...
            const directory = await this.db.getDirectoryBySlug(slug);
     ...
     */
    async purgeDirectoryImages(directoryId) {
        if (!directoryId)
            return 0;
        try {
     ...
     */
    async removeOrphanedImages(directoryId, dirPath) {
        try {
            // Get all images from database for this directory
            const dbImages = await this.db.getImagesByDirectory(directoryId);
            let orphanCount = 0;
     ...
     */
    async updateDirectoryImageCounts() {
        try {
     ...
     */
    async processImage(imagePath, directoryId) {
        ...
        try {
            ...
            const existing = await this.db.getImageByPath(imagePath);
            ...
            if (existing && existing.id) {
                await this.db.updateImage(existing.id, imageData);
            }
            else {
     ...
     */
    async generateThumbnailsForDirectory(slug) {
        const result = {
            imagesProcessed: 0,
            thumbnailsGenerated: 0,
            directoriesCreated: 0,
     ...
     */
    async generateThumbnailsForImage(image) {
    ...
    async processVideo(videoPath, directoryId) {
        try {
            ...
            const existing = await this.db.getImageByPath(videoPath);
            if (existing && existing.id) {
                await this.db.updateImage(existing.id, videoData);
            }
            else {
     ...
     */
    async processDocument(docPath, directoryId) {
        try {
     ...
     */
    async generateThumbnail(imagePath, thumbnailDir, width) {
     ...
     */
    async generateDirectoryConfig(dirPath) {
        const dirName = path.basename(dirPath);
